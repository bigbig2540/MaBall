...
public class HardMode1Player extends Application {
    ...
    public HardMode1Player(String n) {
        player = new PlayerHardMode();
    ...
    private void spawnBall(PlayerHardMode player) {
        ...
        if (waitFinish == 0) {
            ...
            if (randomY == 0) {
                ...
                ballObj.ballSetXandSetY(950, height * 0.5);
            } else if (randomY == 2) {
                ballObj.ballSetXandSetY((width - ballObj.getWidth()) / 2.0, height + 100);
            } else if (randomY == 3) {
                ballObj.ballSetXandSetY(250, height * 0.5);
            }
            ...
            new AnimationTimer() {
                ...
                public void handle(long now) {
                    ...
                    if (randomY == 0) {
                        ...
                        ballObj.setPositionX(ballObj.getPositionX() - randomSpeed);
                        if (ballObj.getPlayerSprite().getBoundsInParent().intersects(player.getPlayerSprite().getBoundsInParent())) {
                            System.out.println(player.getPlayerStatus(0) + player.getPlayerStatus(1) + player.getPlayerStatus(2) + player.getPlayerStatus(3));
                            if (player.getPlayerStatus(randomY).compareTo(ballObj.getBallStatus()) == 0) {
                                waitFinish = 0;
    ...
    private Parent GameOverScene(PlayerHardMode player) throws IOException {
        ...
        playAgainButton.getPlayerSprite().setOnMouseClicked(event -> {
            ...
            BGM.stop();
            try {
                HardMode1Player reStage = new HardMode1Player(player.getPlayerName());
                reStage.start(stage);
